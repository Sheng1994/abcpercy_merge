...
Abc_Ntk_t * Abc_NtkIf( Abc_Ntk_t * pNtk, If_Par_t * pPars )
{
    ...
    // If_Obj_t * pObj; int i;
    // If_Cut_t * pCut;
    // If_ManForEachObj(pNtkNew, pObj, i) {
    //     printf("The delay of node %d best cut: ", pObj->Id );
    //     printf("%f \n ", pObj->CutBest.Delay );
    // }
...
Abc_Ntk_t * Abc_NtkFromIf( If_Man_t * pIfMan, Abc_Ntk_t * pNtk )
{
    ...
    Vec_Int_t * vCover;
    int i, nDupGates;
    // create the new ABC network pNtkNew
    ...
    Abc_NtkForEachCo( pNtk, pNode, i )
    {
        ...
        Vec_Ptr_t * vLeavesUsed = Vec_PtrAlloc(0);
    ...
    /**********************************user define**********************************/
    {
        ...
        If_ManForEachNode(pIfMan, pObj, i) {
            Abc_Obj_t *pMapped = (Abc_Obj_t *)pObj->pCopy;
            if ( pMapped == NULL )
                continue;
            for ( k = 0; k < pMapped->vCoRoots.nSize; k++ ) {
                If_Obj_t *pRoot = If_ManObj( pIfMan, pMapped->vCoRoots.pArray[k] );
                if ( pRoot != NULL && pRoot->pCopy != NULL )
                    pMapped->vCoRoots.pArray[k] = ((Abc_Obj_t *)pRoot->pCopy)->Id;
            }
            for ( k = 0; k < pMapped->vCoFanins.nSize; k++ ) {
                If_Obj_t *pFanin = If_ManObj( pIfMan, pMapped->vCoFanins.pArray[k] );
                if ( pFanin != NULL && pFanin->pCopy != NULL )
                    pMapped->vCoFanins.pArray[k] = ((Abc_Obj_t *)pFanin->pCopy)->Id;
            }
            Vec_IntSort( &pMapped->vCoRoots, NULL );
            Vec_IntSort( &pMapped->vCoFanins, NULL );
        }
    }
