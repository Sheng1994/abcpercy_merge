...
//        assert( pCut->Delay <= pObj->Required + p->fEpsilon );
    ...
    If_ObjForEachCut( pObj->pFanin1, pCut1, k )
    {
        ...
        if (pCut1->nLeaves == 1) {
            pCut1->vNodesInCut = Vec_PtrAlloc(20);
            Vec_PtrPush(pCut1->vNodesInCut, &pObj->pFanin1->Id);
        }
        ...
        // printf("\n");
        //
        // // print the nodes in current cut
        // printf("The nodes in current node %d cut is: ", pObj->Id);
        // for ( i = 0; i < pCut->vNodesInCut->nSize; i++ ) {
        //     printf("%d ", *(int*)pCut->vNodesInCut->pArray[i] );
        // }
        // printf("\n\n");
...
int If_ManPerformMappingRound( If_Man_t * p, int nCutsUsed, int Mode, int fPreprocess, int fFirst, char * pLabel )
{
    ...
    If_ManForEachNode( p, pObj, i ) {
        ...
        // printf("Current Obj Id: %d, BestCut 1st Node: %d\n", pObj->Id,
        //     *(int*)Vec_PtrEntry(pObj->CutBest.vNodesInCut, 0));
    }
