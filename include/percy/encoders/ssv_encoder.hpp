...
namespace percy
{
    class ssv_encoder : public std_cegar_encoder, public enumerating_encoder
    {
        ...
        /*************************Modified for mclsuter*******************************/
        /* Only defined output steps will be considered */
            bool create_output_clauses(const spec& spec)
            {
                ...
                if (spec.nr_nontriv > 1) {
                    ...
                    for (int h = 0; h < spec.nr_nontriv; h++) {
                        for (int i = 0; i < spec.po_steps.size(); i++) {
                            int po_step = spec.po_steps[i] - spec.nr_in;
                            pabc::Vec_IntSetEntry(vLits, i, 
                                    pabc::Abc_Var2Lit(get_out_var(spec, h, po_step), 0));
                        }
                        status &= solver->add_clause(
                                pabc::Vec_IntArray(vLits),
                                pabc::Vec_IntArray(vLits) + spec.po_steps.size());
                    ...
                    // Every step can only have one PO
                    for (int i = 0; i < spec.po_steps.size(); i++) {
                        int ctr = 0;
                        int po_step = spec.po_steps[i] - spec.nr_in;
                        for (int h1 = 0; h1 < spec.nr_nontriv; h1++) {
                            pabc::Vec_IntSetEntry(vLits, ctr++,
                                    pabc::Abc_Var2Lit(get_out_var(spec, h1, po_step), 1));
                            for (int h2 = 0; h2 < spec.nr_nontriv; h2++) {
                                if (h1 == h2) { continue;}
                                pabc::Vec_IntSetEntry(vLits, ctr++,
                                        pabc::Abc_Var2Lit(get_out_var(spec, h2, po_step), 1));
                            }
                        }
                        status &= solver->add_clause(
                                pabc::Vec_IntArray(vLits),
            ...
            {
                auto ret = true;
                // fanin_asgn: the boolean value assign to fanins
                ...
                for (int i = 0; i < spec.nr_steps; i++) {
                    ...
                    // nr_svars_for_i: number of selective variables of step i
                    ...
                    const std::vector<int>& fanin_asgn)
            {
                ...
                // make sure the fanin bit match with the current assigned input value
                ...
                if (opvar_idx > 0) {
                    ...
                    // printf("sel_var=%d, sim_var=%d, tt_var=%d\n", svar, get_sim_var(spec, i, t),
                    //     get_op_var(spec, i, opvar_idx));
                }
            ...
            /*******************************************************************
                Add clauses that realize certain dag with limit on sijk.
            *******************************************************************/
            bool create_dag_clauses(const spec& spec) const {

                auto status = true;
                // Iterate over all steps that require a fan-in configuration
                for (int i = 0; i < spec.nr_steps; i++) {
                    auto ctr = 0;
                    // Get the fan-in indices for the current step from spec.dag
                    int fanin0 = spec.dag[2 * i];       // First fan-in index
                    int fanin1 = spec.dag[2 * i + 1];   // Second fan-in index
                    // both fanins are internal
                    if (fanin0!=0&fanin1!=0) {
                        int svar = spec.calculate_sel_index(i,fanin0,fanin1,spec.nr_in, nr_svar_map);
                        pabc::Vec_IntSetEntry(vLits, ctr++,
                        pabc::Abc_Var2Lit(get_sel_var(svar), 0));
                        status = solver->add_clause(
                                pabc::Vec_IntArray(vLits),
                                pabc::Vec_IntArray(vLits)+1);
                        assert(status);
                    }
                    // only one fanin is external
                    else if (fanin0!=0&fanin1==0|fanin0==0&fanin1!=0) {
                        // determaine which fainin is external
                        int intr_fanin = (fanin0 == 0) ? fanin1 : fanin0;
                        for (int j = 0; j < spec.nr_in; j++) {
                            int svar = spec.calculate_sel_index(i,j,intr_fanin,spec.nr_in, nr_svar_map);
                            pabc::Vec_IntSetEntry(vLits, ctr++,
                            pabc::Abc_Var2Lit(get_sel_var(svar), 0));
                        }
                        status = solver->add_clause(
                ...
                // clauses for all steps with full external PIs
                // always exists external PIs for these nodes
                for (int i = 0; i < spec.nr_steps; i++) {
                    // Get the fan-in indices for the current step from spec.dag
                    int fanin0 = spec.dag[2 * i];       // First fan-in index
                    int fanin1 = spec.dag[2 * i + 1];   // Second fan-in index
                    if (fanin0==0&fanin1==0) {
                        auto ctr = 0;
                        for (int j = 0; j < spec.nr_in; j++) {
                            for (int k = j+1; k < spec.nr_in; k++) {
                                int svar = spec.calculate_sel_index(i,j,k,spec.nr_in, nr_svar_map);
                                pabc::Vec_IntSetEntry(vLits, ctr++,
                                        pabc::Abc_Var2Lit(get_sel_var(svar), 0));
                            }
                        }
            ...
            /*******************************************************************
                Add clauses that ensure each fanin is used.
            *******************************************************************/
            bool create_fanin_clauses(const spec& spec) const {
                auto status = true;
                // Iterate over all steps that require a fan-in configuration
                // for (int j = 0; j < spec.nr_in; j++)
                // {
            ...
            *******************************************************************/
            ...
            create_lex_clauses(const spec& spec)
            {
                ...
                // for (int ik = 0; ik < svar_map.size() - 1; ik++) {
                //     int ikp = svar_map[ik][0];
                //     int ikq = svar_map[ik][1];
                //     std::cout << "The numbers are: " << ikp << " and " << ikq << std::endl;
                // }
                ...
                for (int i = 0; i < spec.nr_steps - 1; i++) {
                    ...
                    for (int j = 0; j < nr_svars_for_i; j++) {
                        ...
                        for (int jp = 0; jp < nr_svars_for_ip; jp++) {
                            ...
                            // Only process the steps with totally same fanins
